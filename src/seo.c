...
void seo_head(const map *site, const map *page, buf *o) {
    ...
    int auto_img = 0;
    if (!*img_src && *get(page, "og_auto")) {
        img_src = get(page, "og_auto");
        auto_img = 1;
    }
    ...
    if (*img) {
        ...
        if (auto_img) {
            meta_prop(o, "og:image:width", "1200");
            meta_prop(o, "og:image:height", "630");
            meta_prop(o, "og:image:type", "image/png");
        }
    ...
    /* Google Analytics 4 (only when google_analytics is set in site.conf) */
    if (*get(site, "google_analytics") && !noindex) {
        buf_puts(o, "<script async src=\"https://www.googletagmanager.com/gtag/js?id=");
        esc_html_s(o, get(site, "google_analytics"));
        buf_puts(o, "\"></script>\n<script src=\"");
        esc_html_s(o, get(site, "base_path"));
        buf_puts(o, "/assets/tantu-analytics.js\" data-id=\"");
        esc_html_s(o, get(site, "google_analytics"));
        buf_puts(o, "\"></script>\n");
    }
...
static void csp(const map *site, buf *o) {
    int ga = *get(site, "google_analytics") != '\0';
    buf_puts(o, "default-src 'self'; img-src 'self' data: https:; style-src 'self'; script-src 'self'");
    if (ga) buf_puts(o, " https://www.googletagmanager.com");
    buf_puts(o, "; connect-src 'self'");
    if (ga) buf_puts(o, " https://*.google-analytics.com https://*.analytics.google.com https://*.googletagmanager.com");
    buf_puts(o, "; font-src 'self'; frame-ancestors 'self'; base-uri 'self'; form-action 'self' https:");
}
...
void seo_htaccess(const map *site, buf *o) {
    ...
    buf_printf(o,
        ...
        "</IfModule>\n",
        get(site, "base_path"), policy.s);
    buf_free(&policy);
}

void seo_headers_file(const map *site, buf *o) {
    buf_puts(o,
